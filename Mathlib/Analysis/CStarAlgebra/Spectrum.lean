...
theorem IsSelfAdjoint.spectralRadius_eq_nnnorm {a : A} (ha : IsSelfAdjoint a) :
  ...
  -- TODO: once we have `ℕ+` powers in semigroups and Gelfand's formula is generalized to non-unital
  -- algebras, then we can skip passing to the unitization here.
  suffices spectralRadius ℂ (a : A⁺¹) = ‖(a : A⁺¹)‖₊ by simpa [Unitization.nnnorm_inr]
  have hconst : Tendsto (fun _n : ℕ => (‖(a : A⁺¹)‖₊ : ℝ≥0∞)) atTop _ := tendsto_const_nhds
  ...
  rw [Function.comp_apply, (ha.inr ℂ).nnnorm_pow_two_pow, ENNReal.coe_pow, ← rpow_natCast,
    ← rpow_mul]
  simp
...
theorem IsStarNormal.spectralRadius_eq_nnnorm (a : A) [IsStarNormal a] :
  ...
  -- algebras, then we can skip passing to the unitization here.
  suffices spectralRadius ℂ (a : A⁺¹) = ‖(a : A⁺¹)‖₊ by simpa [Unitization.nnnorm_inr]
  have : IsStarNormal (a : A⁺¹) := inferInstance
  refine (ENNReal.pow_right_strictMono two_ne_zero).injective ?_
  have heq :
    (fun n : ℕ => (‖((a⋆ * a) ^ n : A⁺¹)‖₊ : ℝ≥0∞) ^ (1 / n : ℝ)) =
      (fun x => x ^ 2) ∘ fun n : ℕ => (‖(a ^ n : A⁺¹)‖₊ : ℝ≥0∞) ^ (1 / n : ℝ) := by
    funext n
    rw [Function.comp_apply, ← rpow_natCast, ← rpow_mul, mul_comm, rpow_mul, rpow_natCast, ←
      coe_pow, sq, ← nnnorm_star_mul_self, inr_star, Commute.mul_pow (star_comm_self' (a : A⁺¹)), star_pow]
  have h₂ :=
    ((ENNReal.continuous_pow 2).tendsto (spectralRadius ℂ (a : A⁺¹))).comp
      (spectrum.pow_nnnorm_pow_one_div_tendsto_nhds_spectralRadius (a : A⁺¹))
  rw [← heq] at h₂
  convert! tendsto_nhds_unique h₂
    (pow_nnnorm_pow_one_div_tendsto_nhds_spectralRadius (a⋆ * a : A⁺¹))
  rw [inr_star, (IsSelfAdjoint.star_mul_self (a : A⁺¹)).spectralRadius_eq_nnnorm, sq,
    nnnorm_star_mul_self, coe_mul]
