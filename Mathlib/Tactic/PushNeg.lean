...
partial def pushNegTransformStep (expr : Expr) : MetaM (Simp.Result) := do
  ...
  | some expr' =>
    ...
    | (`Not, #[e]) =>
      return { expr := e, proof? := mkAppM ``not_not_eq #[e] }
    | (`And, #[p, q]) =>
      return { expr := mkForall `_ default p (mkNot q), proof? := mkAppM ``not_and_eq #[p, q]}
    | (`Or, #[p, q]) =>
      return { expr := mkAnd (mkNot p) (mkNot q), proof? := mkAppM ``not_or_eq #[p,q] }
    | (`Exists, #[_, e]) =>
      match e with
        | (Expr.lam n typ bo bi) => do
        return { expr := expr, proof? := none }
